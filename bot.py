...
def choose_move(maze):# The idea is to find the move down or right that jumps the most squares. If not, then just move 
    ...
    RJ = check_collision(col+1, row, "right", maze, True) # right jump legal
    RM = check_collision(col+1, row, "right", maze, False) # right move legal
    ...
    elif(down_jump >= right_jump and DJ and down_jump != 0): # jump down if it jump more than a right jump. = in case right jump is not valid on tie
        return 0x16 #return hex for down jump.
        
    else: #No hopping found and down is not valid. We must check walls
        if bottom_open and DM: #if there is no wall and no player lower
            #print("MD")
            return 0x12 # move down
        elif bottom_open == 0 and DJ: #wall but no player
            return 0x16
        elif right_open and RM:
            return 0xf
        else:
            return 0x13 # this might return if there are no valid moves that bring you closer to the exit
